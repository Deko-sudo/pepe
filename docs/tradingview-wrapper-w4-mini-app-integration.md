...
The W4 deterministic parent-browser fixture is test-only and contains no TradingView dependency. It serves the actual Mini App `/markets` parent at `127.0.0.1:4180`, an authenticated API fixture at `:4181`, a synthetic lifecycle-only wrapper at `:4182`, and a wrong-origin attacker at `:4183`. The suite verifies exact iframe attributes and backend URL usage, capability/configuration requests, quote/candle suppression, wrong-origin rejection, and stale-frame rejection after selection replacement. It runs in the existing TradingView wrapper CI job and has no production CSP, hosting, DNS, TLS, or provider-activation impact. W5 CSP/kill-switch/rollback controls, W6 workflow changes, W7 production gates, and Stage 9 remain out of scope. Written TradingView confirmation remains mandatory before production activation.
