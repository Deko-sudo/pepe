...
W1 and W2 are merged. W3 is delivered by its current backend-only PR and awaits owner merge; W4–W7 are not started. W3 adds a fail-closed authenticated configuration contract for the isolated wrapper's 18 canonical local/test routes. It makes no Mini App integration, parent-CSP change, backend/provider network request, production activation, or Stage 9 change. Production remains blocked pending mandatory written TradingView confirmation and later W4–W7 gates.
