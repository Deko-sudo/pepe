...
export function Markets() {
  ...
  return <div className="flex h-full flex-col gap-4 overflow-y-auto p-4 pb-24">
    ...
    {hasSession && !capabilities.isLoading && !machineReadableMarketData ? <section className="card" role="status" aria-live="polite"><h2 className="text-sm font-medium text-text-secondary">Рыночные данные недоступны</h2><p className="mt-2 text-sm">Внешний источник графика ещё не настроен. Числовые котировки и свечи сейчас не отображаются.</p>{capabilities.isError ? <button className="mt-2 underline" onClick={retry}>Повторить</button> : null}</section> : null}
