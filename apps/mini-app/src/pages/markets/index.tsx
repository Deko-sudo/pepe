...
import { getAssets, getCandles, getEmbeddedChartConfig, getMarketDataCapabilities, getQuote, type Candle, type Timeframe } from "@/shared/api/market";
...
export function Markets() {
  ...
  const embeddedChartAvailable = capabilities.data?.embedded_chart_available === true;
  const canLoadMarket = hasSession && Boolean(selectedSlug) && machineReadableMarketData;
  const chartConfig = useQuery({ queryKey: ["embedded-chart-config", selectedSlug, timeframe], queryFn: () => getEmbeddedChartConfig(selectedSlug!, timeframe), enabled: hasSession && Boolean(selectedSlug) && embeddedChartAvailable, retry: false });
  ...
  const retry = () => { void catalog.refetch(); void capabilities.refetch(); if (canLoadMarket) { void quote.refetch(); void candles.refetch(); } if (embeddedChartAvailable) void chartConfig.refetch(); };
  ...
  return <div className="flex h-full flex-col gap-4 overflow-y-auto p-4 pb-24">
    ...
    {hasSession && embeddedChartAvailable ? <><section className="card"><div className="flex flex-wrap gap-2" role="group" aria-label="Таймфрейм графика">{timeframes.map((value) => <button key={value} aria-pressed={timeframe === value} onClick={() => setTimeframe(value)} className={`touch-target rounded-lg border px-3 text-sm ${timeframe === value ? "border-brand-primary text-brand-primary" : "border-border-subtle"}`}>{value}</button>)}</div></section>{chartConfig.isLoading ? <section className="card" role="status">Загрузка конфигурации графика…</section> : chartConfig.isError ? <section className="card" role="alert">График провайдера недоступен. <button className="underline" onClick={retry}>Повторить</button></section> : chartConfig.data ? <EmbeddedMarketChart config={chartConfig.data} onRetry={retry} /> : null}</> : null}
    {hasSession && !capabilities.isLoading && !capabilities.isError && !machineReadableMarketData && !embeddedChartAvailable ? <section className="card" role="status" aria-live="polite"><h2 className="text-sm font-medium text-text-secondary">Рыночные данные недоступны</h2><p className="mt-2 text-sm">Внешний источник графика ещё не настроен. Числовые котировки и свечи сейчас не отображаются.</p></section> : null}
