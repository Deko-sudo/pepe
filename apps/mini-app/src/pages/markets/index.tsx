...
export function Markets() {
  ...
  const canLoadMarket = hasSession && Boolean(selectedSlug);
  const quote = useQuery({ queryKey: ["quote", selectedSlug], queryFn: () => getQuote(selectedSlug!), enabled: canLoadMarket });
  const candles = useQuery({ queryKey: ["candles", selectedSlug, timeframe], queryFn: () => getCandles(selectedSlug!, timeframe), enabled: canLoadMarket });
  ...
  const retry = () => { void catalog.refetch(); if (canLoadMarket) { void quote.refetch(); void candles.refetch(); } };
