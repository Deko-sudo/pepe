...
import { getAssets, getCandles, getQuote, type Candle, type Timeframe } from "@/shared/api/market";
...
function decimalToScaled(value: string): bigint {
  ...
  const digits = `${whole.replace("-", "")}${fraction.padEnd(12, "0").slice(0, 12)}`.replace(/^0+/, "") || "0";
  return sign * BigInt(digits);
}

function CandleChart({ candles }: { candles: Candle[] }) {
  const points = useMemo(() => {
    const values = candles.map((candle) => decimalToScaled(candle.close));
...
export function Markets() {
  ...
  const quote = useQuery({ queryKey: ["quote", selectedSlug], queryFn: () => getQuote(selectedSlug!), enabled: Boolean(selectedSlug) });
  const candles = useQuery({ queryKey: ["candles", selectedSlug, timeframe], queryFn: () => getCandles(selectedSlug!, timeframe), enabled: Boolean(selectedSlug) });
  const currentQuote = quote.data?.items.find((item) => item.slug === selectedSlug);
  const quoteState = quote.data?.unavailable.includes(selectedSlug ?? "") ? "Котировка временно недоступна" : quote.data?.not_found.includes(selectedSlug ?? "") ? "Инструмент не найден" : null;
  const retry = () => { void catalog.refetch(); void quote.refetch(); void candles.refetch(); };
  ...
  return <div className="flex h-full flex-col gap-4 overflow-y-auto p-4 pb-24">
    ...
    {selectedSlug ? <><section className="card" aria-live="polite"><h2 className="text-sm font-medium text-text-secondary">Текущая котировка</h2>{quote.isLoading ? <p className="mt-2">Загрузка…</p> : currentQuote ? <><p className="mt-2 text-3xl font-bold tabular-nums">{currentQuote.price}</p><p className="mt-2 text-xs text-text-secondary">Источник: {currentQuote.provenance.source_label}; обновлено {new Date(currentQuote.received_at).toLocaleString()}</p></> : <p className="mt-2 text-sm">{quoteState ?? "Котировка недоступна"}</p>}{quote.isError ? <button className="mt-2 underline" onClick={retry}>Повторить</button> : null}</section>
