...
Экран «Рынки» загружает доступный пользователю каталог через Caddy/API и предлагает выбрать
инструмент из каталога. Для выбранного инструмента он показывает текущую котировку с
provenance и историю закрытых свечей. Управление историей поддерживает только `1m`, `5m`,
`15m`, `1h`, `4h` и `1d`; график строится из фактически полученных candle-закрытий.

Запросы требуют валидной Telegram-сессии. Интерфейс явно показывает загрузку, пустой каталог
или историю, недоступную котировку, не найденный инструмент, необходимость авторизации и
сетевую ошибку с повтором запроса. Для локальной smoke-проверки запустите `docker compose up -d`,
откройте Mini App через локальный Caddy и используйте штатный production-signed Telegram session
flow; после проверки остановите стек `docker compose down --remove-orphans`.

Stage 8 намеренно не добавляет торговые сигналы, аналитику, процентные изменения, объём или
иные рыночные метрики, которых API не предоставляет.
